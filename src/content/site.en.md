...
::project{#vox title="Vox" meta="Noah Labs" link="Read more →"}
...
::project{#cv title="Curriculum vitæ" meta="React · Tailwind" link="Open the CV →"}
...
::project{#site title="This site" meta="React · Vite" link="Source →"}
...
::project{#knock title="Engine knock detection" meta="IEEE · UFSC" link="Read the paper ↗"}
...
::topic{#delivery-guarantees label="Transactional and delivery guarantees"}
...
::topic{#ergonomics label="Developer ergonomics and tooling you can reason about"}
Build systems you can explain. Errors that say what to do next.
::

::topic{#perf-abstractions label="Performance implications of language abstractions"}
What that iterator actually compiles to, and whether it matters here.
::

::topic{#cpu-cache label="CPU cache behaviour and memory-efficient code patterns"}
...
::topic{#complexity label="Algorithmic complexity and scalability limits"}
Where the curve bends, and what the constant factor is doing in the meantime.
::

::topic{#agent-evals label="Agent evaluation and failure modes"}
...
::topic{#codesign label="Hardware-software codesign"}
